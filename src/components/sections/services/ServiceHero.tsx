...
function TitleLink({ children }: TitleLinkProps) {
  ...
  return (
    <h1
      ref={linkRef}
      className={`text-5xl font-bold tracking-tight sm:text-6xl md:text-7xl lg:text-8xl cursor-pointer transition-all duration-300 transform-gpu perspective-1000 ${
        isHovered ? "z-10" : ""
      }`}
      style={{
      ...
      <div className="relative z-10 h-full flex flex-col justify-between p-8 md:p-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end">
          <div className="text-left mb-8 md:mb-0">
            <TitleLink>{service.heroTitle}</TitleLink>
          </div>

          <div className="text-right text-gray-300 text-xs md:text-sm max-w-xs">
            {service.heroSubtitle && (
              <p className="mb-2 font-semibold text-white text-xl md:text-2xl">
                {service.heroSubtitle}
              </p>
            )}
            <p className="mb-4 text-gray-400 text-base md:text-lg leading-relaxed">
